...
class ResponseService:
    ...
    def submit(self, survey_id, respondent, answers):
        ...
        for ans in answers:
            ...
            clean_answers.append({"question_id": qid,"answer": str(answer_val)})

        response_data = {"survey_id": survey_id,"respondent": respondent or "anonymous","answers": clean_answers}
    ...
    def pageinate_responses(self, survey_id, page=1, per_page=10):
        all_responses = self.get_responses(survey_id=survey_id)
        return self.response_repo.paginate(all_responses, page=page, per_page=per_page)
